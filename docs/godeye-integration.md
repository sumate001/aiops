...
### Health score บน stream ที่ถูกกรองมาแล้ว
health score ใช้ **จำนวน entry ทั้งหมดก่อน filter** เป็นตัวหาร และ cap deduction ต่อ severity
(warn สูงสุด -35, error สูงสุด -70, anomaly สูงสุด -30) — ดังนั้นถ้า GodEye ส่งมาเฉพาะ warn+
(กรองจากต้นทาง) host จะไม่ถูกปักเป็น health 0/critical โดยอัตโนมัติ:
- warn อย่างเดียว → ลึกสุดโซน warning (~65 ถ้ามี anomaly ร่วม)
- ต้องมี error หรือ anomaly แรงๆ ถึงจะลงไป critical
- ถ้าไม่มี error เลย `top_errors` จะ **fallback เป็นข้อความ warn** เพื่อให้ MiroFish /
  LLM judge / A2 ยังมี log text จริงเป็นหลักฐาน
...
> ⚠️ **Unit-mismatch guard**: threshold หน่วย percent จะถูกใช้เฉพาะเมื่อค่านั้น "เป็นเปอร์เซ็นต์จริง"
> — ข้าม metric ที่ (ก) ส่ง `unit` มาแล้วไม่ใช่ percent, (ข) ชื่อบอกว่าเป็น counter/bytes
> (`_total`, `_bytes`, `_count`, `_limit`, `_seconds`, `_switches`), หรือ (ค) ค่าอยู่นอกช่วง 0-100
> เช่น `system_memory_usage_bytes=2.6e9` จะ**ไม่**ถูกตีเป็น "เกิน 95%" อีกต่อไป
> ถ้าอยากให้ metric ดิบพวกนี้มี threshold ให้เพิ่ม key เฉพาะใน `metric_thresholds` พร้อมค่า
> warn/critical ในหน่วยจริงของมัน (ไม่ใช่ percent)
> **แนะนำ**: ส่ง metric แบบ percent พร้อม `"unit": "percent"` จะแม่นสุด
...
```json
{
  ...
  "health_score": 30.0,
  "status": "critical",
  "summary": "1 critical, 0 warning, 0 ok hosts out of 1. | Primary issue on pos-db-01: 1) <root cause> 2) <ผลกระทบต่อเนื่อง> | Why: <เหตุผลย่อ> | Research: <สรุป A2 ย่อ> | Recommended first step: <ขั้นแรก>",
  "hosts": [
    {
      "host": "pos-db-01",
      ...
      "synthesis": {
        ...
        "reasoning": "2-4 ประโยคอธิบายว่าหลักฐานชี้ root cause นี้อย่างไร (null เมื่อ method=rule)"
      },
      "enrichment": {
...
`synthesis.root_cause_chain` + `synthesis.fix_steps` + `synthesis.confidence` + `synthesis.reasoning`
(สาเหตุ + วิธีแก้ + คำอธิบาย), `prediction.risk_level` (ทำนาย incident),
`enrichment.answer/sources` (ความรู้จาก web)

> 💡 **การแสดงผลฝั่ง GodEye UI**: `summary` ถูกจัดเป็น plain text บรรทัดเดียว (คั่นด้วย ` | `,
> ล้าง markdown แล้ว) เหมาะกับช่องแสดงผลสั้นๆ — ถ้าต้องการหน้ารายละเอียด แนะนำ render จาก
> field ที่มีโครงสร้าง: `root_cause_chain` (list), `fix_steps` (list), `reasoning` (ข้อความ),
> `enrichment.answer` (markdown เต็ม เหมาะกับ markdown renderer)
