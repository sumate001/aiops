...
- **Cache + cooldown**: คำตอบ A2 ถูก cache ต่อ query 6 ชั่วโมง และมี cooldown 60 วินาที
  ระหว่าง search จริง (ค่าคงที่ `_CACHE_TTL`/`_MIN_SEARCH_INTERVAL` ใน `perplexica_client.py`) —
  จำเป็นเพราะ ingest ต่อเนื่องจาก GodEye เคยทำให้ DuckDuckGo/Brave CAPTCHA/429-ban ตัว SearXNG
- **Engine ของ SearXNG**: DuckDuckGo / Brave / Startpage ถูกปิดใน `/etc/searxng/settings.yml`
  ของ container (โดนแบนง่ายเมื่อยิงถี่) — ใช้ Mojeek/Qwant แทน; แก้ไฟล์ต้องใช้ `docker exec -i`
...
log-analyzer :8200
    │
    ├─ filter + group by host (+ metric threshold anomalies)
    ├─ Phase 1  A1: window_stats + log-ml Isolation Forest score
    ├─ Phase 2  A3: MiroFish 5-frame analysis (+ per-frame LLM insight)
    ├─ Phase 3  AA rule pass (เร็ว ไม่ใช้ LLM — top_frame ใช้สร้าง A2 query)
    ├─ Phase 4  A2: Perplexica enrichment (sequential, cache 6h + cooldown 60s)
    ├─ Phase 5  AA LLM judge — เห็น error/warn msgs + insights + ค่า metric + ผล A2
...
```json
{
  "health_score": 30.0,
  "status": "critical",
  "summary": "1 critical, 0 warning, 0 ok hosts out of 1. | Primary issue on pos-01: 1) ... 2) ... | Why: ... | Research: ... | Recommended first step: ...",
  "hosts": [
    {
      "host": "pos-01",
      "health_score": 30.0,
...
- `synthesis.method: "llm"` จะมี `reasoning` (2-4 ประโยค อธิบายว่าหลักฐานชี้ root cause นี้อย่างไร);
  rule fallback ได้ `reasoning: null`
- **Health score**: ตัวหารคือจำนวน log ทั้งหมดก่อน severity filter และ deduction ถูก cap
  (warn สูงสุด -35, error สูงสุด -70, anomaly สูงสุด -30) — stream ที่ถูกกรองมาเป็น warn+
  อยู่แล้ว (เช่นจาก GodEye) จะไม่โดนปักที่ 0/critical โดยอัตโนมัติอีก
- `summary` เป็น **บรรทัดเดียว** คั่นส่วนด้วย ` | ` และล้าง markdown แล้ว (เพื่อ UI ที่ render
  plain text) — รายละเอียดเต็มอยู่ใน `synthesis.*` และ `enrichment.answer` (markdown)
