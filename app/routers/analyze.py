...
# ── Phase 1b: A1b — Chronos seasonal forecast ──
# Batched across hosts on purpose: one model call for N hosts costs about what
# one host costs, and this runs on every analysis.
async def _phase1b_forecast(states: list[_HostState]) -> None:
    if not config.forecast.enabled:
        return

    cfg = config.forecast
    jobs: list[tuple[_HostState, str, list[float], float]] = []
    for st in states:
        for metric in cfg.metrics:
            try:
                series = baseline_store.get_hourly_series(
                    st.hostname, metric, hours=max(cfg.min_history_hours * 2, 336)
                )
            except ValueError:
                continue
            # Too little history is not an error — forecasting a daily rhythm
            # needs to have seen it repeat, and a confident band drawn from two
            # points is worse than no band at all.
            # Drop the hour in progress. Its bucket is still filling, so a
            # summed metric reads low against a full-hour forecast and would
            # breach the lower bound at the top of every hour — a false positive
            # generated by the clock, not by the host. Comparing the last
            # *complete* hour costs up to an hour of lag; A1 already covers the
            # immediate point anomalies, A1b's job is the seasonal comparison.
            current_bucket = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:00:00Z")
            if series and series[-1][0] >= current_bucket:
                series = series[:-1]
            if len(series) < cfg.min_history_hours:
                continue
            values = [v for _, v in series]
            jobs.append((st, metric, values[:-1], values[-1]))

    if not jobs:
        logger.info("A1b forecast — skipped (ประวัติไม่ถึง %dh)", cfg.min_history_hours)
        return

    try:
        quantiles = await asyncio.wait_for(
            asyncio.to_thread(
                forecast.get_forecaster().predict,
                [values for _, _, values, _ in jobs],
                cfg.prediction_length,
            ),
            timeout=cfg.timeout_seconds,
        )
    except (asyncio.TimeoutError, Exception) as exc:
        logger.warning("A1b forecast failed, continuing without: %s", exc)
        return

    for (st, metric, _values, actual), q in zip(jobs, quantiles):
        p10, p50, p90 = (float(x) for x in q[0])
        breach, magnitude = forecast.breach_of(actual, p10, p50, p90)
        st.forecast.append(ForecastBand(
            metric=metric, p10=round(p10, 3), p50=round(p50, 3), p90=round(p90, 3),
            actual=round(actual, 3), breach=breach, breach_magnitude=magnitude,
        ))
        metrics.record_forecast(st.hostname, metric, p10, p50, p90, breach)

    breached = [f.metric for st in states for f in st.forecast if f.breach]
    logger.info("A1b forecast — %d bands, breached: %s",
                sum(len(st.forecast) for st in states), breached or "none")
...
async def analyze(req: AnalyzeRequest) -> AnalyzeResponse:
    ...
    # ── Phase 1b: A1b Chronos forecast — batched across all hosts ──
    logger.info("=== Phase 1b: A1b forecast — %d hosts ===", len(states))
    await _phase1b_forecast(states)
